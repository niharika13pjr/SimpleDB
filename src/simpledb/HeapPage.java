...
public class HeapPage implements Page {
	...
	protected Tuple readTuple(byte[] data, int slotId) throws NoSuchElementException {
		...
		DataInputStream in = new DataInputStream(new ByteArrayInputStream(data, readInt(data,4+4*slotId),data.length - readInt(data,4+4*slotId)));
		try{
			tup = readTuple(in,readInt(data,4+4*slotId));
		}
	...
	public byte[] getPageData() {
		...
		// some code goes here
		//throw new UnsupportedOperationException("Implement this");
		int length = BufferPool.PAGE_SIZE;
		byte[] result=new byte[length];
		writeInt(result,0,tupleSize);
		int offset=4;
		for(int i=0;i<tupleSize;i++)
		{
			byte[] byteArray=toByteArray(tuples.get(i));
			length = length - byteArray.length;
			System.arraycopy(byteArray, 0, result, length - 1, byteArray.length);
			writeInt(result,offset,length - 1);
			offset =+4;
		}
		return result;
	}
